...
import { GlassCard, Badge, ShimmerButton } from './ui/Primitives';
...
export const DirectoryTab: React.FC<DirectoryTabProps> = ({ contacts, loading }) => {
  ...
  return (
    <div className="space-y-6 pb-10">
      <div>
        <h1 className="font-display text-2xl font-black text-white tracking-tight">Contact Directory</h1>
        <p className="text-sm text-slate-400">Faculty, staff, HoDs, wardens, and placement coordinators.</p>
      </div>
      ...
      <GlassCard className="p-4 space-y-3" hover={false}>
        <div className="flex gap-3 flex-col sm:flex-row">
          ...
          <select
            ...
            className="bg-slate-900/60 text-sm text-slate-300 px-3 py-2.5 rounded-xl border border-white/5 focus:outline-none focus:border-blue-500/30 transition sm:w-56"
          >
            {departments.map((d, i) => (
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 text-center gap-4">
          ...
          <div>
            <p className="text-slate-300 font-semibold">No contacts found</p>
          ...
          <AnimatePresence>
            {filtered.map((c, i) => {
              ...
              return (
                ...
                >
                  <GlassCard className="p-5 group" glow="blue">
                    {/* Card header */}
                    <div className="flex items-start gap-3 mb-4">
                      <div className={cn(
                        'w-10 h-10 rounded-xl flex items-center justify-center text-white font-bold text-sm flex-shrink-0 bg-gradient-to-br shadow-inner-light',
                        avatarGradient
                      )}>
                        {initials(c.name)}
                      </div>
                      <div className="flex-1 min-w-0">
                        <h3 className="font-bold text-sm text-white group-hover:text-blue-200 transition-colors truncate">
                          {c.name}
                        </h3>
                        <p className="text-2xs text-blue-400/80 font-medium truncate">{c.designation}</p>
                      </div>
                    </div>
                    ...
                    <div className="space-y-1.5 pt-3 border-t border-white/5">
                      ...
                      <div className="flex items-center justify-between group/row">
                        <a
                          href={`mailto:${c.email}`}
                          className="flex items-center gap-2 text-2xs text-slate-400 hover:text-blue-400 transition-colors truncate pr-2"
                        >
                        ...
                        <button
                          ...
                          className="opacity-0 group-hover/row:opacity-100 p-1 rounded text-slate-600 hover:text-slate-300 transition-all duration-150 flex-shrink-0"
                        >
                          {copied === `${i}-email` ? <Check className="h-3 w-3 text-emerald-400" /> : <Copy className="h-3 w-3" />}
                        </button>
                      </div>
                      ...
                      {c.phone && (
                        <div className="flex items-center justify-between group/row">
                          <a
                            href={`tel:${c.phone}`}
                            className="flex items-center gap-2 text-2xs text-slate-400 hover:text-emerald-400 transition-colors"
                          >
                            <Phone className="h-3 w-3 flex-shrink-0 text-slate-600" />
                            <span>{c.phone}</span>
                          </a>
                          <button
                            ...
                            className="opacity-0 group-hover/row:opacity-100 p-1 rounded text-slate-600 hover:text-slate-300 transition-all duration-150 flex-shrink-0"
                          >
                            {copied === `${i}-phone` ? <Check className="h-3 w-3 text-emerald-400" /> : <Copy className="h-3 w-3" />}
                          </button>
                        </div>
                      )}
