  ...
  return (
    <div className="space-y-5 pb-10 relative">
      ...
      <div className="relative z-10">
        <h1 className="font-display text-xl font-semibold gradient-text bg-gradient-to-r from-amber-700 to-amber-800 tracking-tight">Contact Directory</h1>
        <p className="text-xs text-stone-500 mt-0.5">Faculty, staff, HoDs, and placement coordinators.</p>
      </div>
      ...
      <div className="space-y-2">
        <div className="flex gap-2 flex-col sm:flex-row">
          ...
          <select
            ...
            className="bg-[#FFFDF9] text-sm text-stone-700 px-3 py-2 rounded-lg border border-[#E3D9C6] focus:outline-none focus:border-amber-500 focus:ring-1 focus:ring-amber-500/20 transition sm:w-56 appearance-none cursor-pointer"
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-16 text-center gap-3">
          ...
          <div>
            <p className="text-stone-700 font-medium text-sm">No contacts found{search ? ` for "${search}"` : ''}</p>
            <p className="text-xs text-stone-500 mt-0.5">Try a different search or department filter</p>
          </div>
                  ...
                  <Card className="p-4 group">
                    <div className="flex items-start gap-3 mb-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <h3 className="font-medium text-sm text-stone-800 group-hover:text-amber-700 transition-colors truncate">
                          {c.name}
                        </h3>
                        <p className="text-xs text-stone-500 truncate">{c.designation}</p>
                      </div>
                    </div>
                    ...
                    <div className="space-y-1 pt-2 border-t border-[#E8E2D5]">
                      <div className="flex items-center justify-between group/row">
                        <a
                          href={`mailto:${c.email}`}
                          className="flex items-center gap-1.5 text-xs text-stone-500 hover:text-amber-700 transition-colors truncate pr-2"
                        >
                      ...
                      {c.cabin && (
                        <div className="flex items-center gap-1.5 text-xs text-stone-500">
                          <Building2 className="h-3 w-3 flex-shrink-0 text-stone-400" />
                          <span>{c.cabin}</span>
                        </div>
                      )}
                    </div>
