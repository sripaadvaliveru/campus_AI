...
export const AnalyticsTab: React.FC<AnalyticsTabProps> = ({ analyticsData, loading, modelName }) => {
  ...
  return (
    <div className="space-y-6 pb-10">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-5">
        <Card className="lg:col-span-8 p-5" hover={false}>
          <div className="flex items-center gap-2 mb-4">
            ...
            <h2 className="font-medium text-sm text-slate-800">Query Volume — Last 7 Days</h2>
            <Badge variant="blue" className="ml-auto">Live</Badge>
          </div>
          {chartData.length === 0 ? (
            <p className="text-xs text-slate-500 text-center py-10">No data yet.</p>
          ) : (
        ...
        <Card className="lg:col-span-4 p-5" hover={false}>
          <div className="flex items-center gap-2 mb-4">
            ...
            <h2 className="font-medium text-sm text-slate-800">Top Categories</h2>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-5">
        <Card className="lg:col-span-4 p-5" hover={false}>
          <h2 className="font-medium text-sm text-slate-800 mb-3">Popular Queries</h2>
              ...
              : popular_queries.map((q, i) => (
                <div key={i} className="py-2 flex items-center justify-between gap-2">
                  <div className="flex items-center gap-2 min-w-0">
                    <span className="text-2xs font-medium text-slate-400 w-4 flex-shrink-0">{i + 1}</span>
                    <span className="text-xs text-slate-600 truncate">"{q.user_query}"</span>
                  </div>
        ...
        <Card className="lg:col-span-8 p-5" hover={false}>
          <h2 className="font-medium text-sm text-slate-800 mb-3">Recent Queries</h2>
          <div className="overflow-x-auto">
            <table className="w-full text-xs">
              <thead>
                <tr className="border-b border-slate-100 text-slate-500 text-2xs">
              ...
              <tbody className="divide-y divide-slate-100">
                {recent_queries.length === 0
                  ? <tr><td colSpan={3} className="py-6 text-center text-slate-500">No logs yet.</td></tr>
                  : recent_queries.map(q => (
                    <tr key={q.id} className="hover:bg-slate-50 transition-colors">
                      <td className="py-2 pr-4 text-slate-600 truncate max-w-[240px]" title={q.user_query}>
                        "{q.user_query}"
                      </td>
